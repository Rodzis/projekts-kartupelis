...
def izveidot_laukumu():
    ...
    global laika_label, atlikusas_sekundes, dzivibu_label, dzivibas

    spēle_beigusies = False
    Laukums = Tk()
    Laukums.title("Kartupelis")

    if grutibas_pakape == "Viegls":
        atlikusas_sekundes = 120
        dzivibas = 20
    elif grutibas_pakape == "Vidējs":
    ...
    dzivibu_label = Label(Laukums, text=f"Dzīvības: {dzivibas} ❤️", fg="black", font=("Arial", 12))
...
def izveidot_izvelni():
    global sākums, grutibas_pakape
    sākums = Tk()
    sākums.title("Menu")
    sākums.geometry("900x600")

    sākums_rāmis = Frame(sākums)
    sākums_rāmis.pack(pady=50)

    Label(sākums_rāmis, text="Izvēlies grūtības pakāpi:").pack(pady=10)
    grutibas_var = StringVar(value="Vidējs")

    Radiobutton(sākums_rāmis, text="Viegls", variable=grutibas_var, value="Viegls").pack()
    Radiobutton(sākums_rāmis, text="Vidējs", variable=grutibas_var, value="Vidējs").pack()
    Radiobutton(sākums_rāmis, text="Grūts", variable=grutibas_var, value="Grūts").pack()

    def sākt_ar_grūtību():
        global grutibas_pakape
        grutibas_pakape = grutibas_var.get()
        sākums.destroy()
        kartupeļa_novietojums()
        izveidot_laukumu()

    Button(sākums_rāmis, text="SPĒLĒT", command=sākt_ar_grūtību).pack(pady=20)
    Button(sākums_rāmis, text="IZIET", command=sākums.destroy).pack(pady=20)
...
def minēt(rinda, kolonna):
    ...
    if (rinda, kolonna) in kartupeļa_pozīcija:
        ...
        # ✅ Pievieno 1 dzīvību par trāpījumu
        dzivibas += 1
        dzivibu_label.config(text=f"Dzīvības: {dzivibas} ❤️")
        ...
        if dzivibas <= 0:
            rezultāta_label.config(text="Tu zaudēji visas dzīvības!", fg="orange")
            atspējot_pogas()
